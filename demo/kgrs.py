...
class Dataloader:
    def __init__(self, train_pos, train_neg, kg_lines, n_user, n_item, train_batch_size: int = 128, neg_rate: float = 2):
        ...
        self.n_item = n_item
        self._load_ratings()
        self.known_neg_dict = []  # Save the currently known negative
    ...
    def _convert_kg(self, lines):
        ...
        kg = []
        rel_dict = {}
        # Get the absolute path to relation2id.txt in the demo directory
        relation_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'relation2id.txt')
        for line in open(relation_file, encoding='utf8').readlines():
        ...
        for line in lines:
            ...
            entity_set.add(head)
            entity_set.add(tail)
        print(f'number of entities (containing items): {len(entity_set)}')
        print(f'number of relations: {len(rel_dict)}')
        return kg, rel_dict, max(entity_set) + 1 if len(entity_set) > 0 else 0
...
class TransE(torch.nn.Module):
    ...
    def forward(self, head, rel, tail) -> torch.Tensor:
        ...
        if self.l1:
            score = torch.sum(torch.abs((head_emb + rel_emb) - tail_emb), dim=-1, keepdim=True)
        else:
            score = torch.sum(torch.square((head_emb + rel_emb) - tail_emb), dim=-1, keepdim=True)
        return -score
    ...
    def ctr_eval(self, eval_batches: List[np.array]):
        ...
        for batch in eval_batches:
            rel = [self.dataloader.rel_dict['feedback_recsys']] * len(batch[0])
            score = torch.squeeze(
                self.forward(batch[0] + self.dataloader.n_entity, rel, batch[1]),
                dim=-1
            )
    ...
    def top_k_eval(self, users: List[int], k: int = 5):
        ...
        sorted_list = []
        for user in users:
            head = [user + self.dataloader.n_entity] * len(item_list)
            rel = [self.dataloader.rel_dict['feedback_recsys']] * len(item_list)
            tail = item_list
            scores = torch.squeeze(self.forward(head, rel, tail), dim=-1)
            score_idx = np.argsort(scores.cpu().detach().numpy())[::-1]
            sorted_items = []
            for idx in score_idx:
                if len(sorted_items) >= k:
                    break
                if user not in train_user_pos_item or item_list[idx] not in train_user_pos_item[user]:
                    sorted_items.append(item_list[idx])
    ...
    def train_TransE(self, epoch_num: int, output_log=False):
        ...
        for epoch in tqdm(range(epoch_num)):
            ...
            if output_log:
                print(f"The loss after epoch {epoch}: {np.mean(losses)}")
